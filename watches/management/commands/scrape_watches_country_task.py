...
from watches.models import Task, Watch
...
class Command(BaseCommand):
    ...
    VALID_COUNTRY_IDS = [
        ...
        "UK",
        "US",
    ]  # Add more country IDs if needed
    ...
    def handle(self, *args, **kwargs):
        ...
        brand = str(task.brand).lower()  # Convert brand to lowercase string
        country_id = str(task.country).upper()  # Convert country ID to uppercase string
        ...
        if country_id not in self.VALID_COUNTRY_IDS:
            ...
            self.stdout.write(
                self.style.NOTICE(
                    f"Valid Country IDs: {', '.join(self.VALID_COUNTRY_IDS)}"
                )
            )
            return
            ...
            for i in range(1, int(total_pages) + 1):
                if i <= max_pages:
                    if i == 1:
                        ...
                        link = link.replace(
                            "&sortorder=0", f"&showpage={i}&sortorder=0"
                        )  # Add showpage parameter
                    label = i
